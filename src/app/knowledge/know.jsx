"use client";
import questions2 from "../data2/knowledgedata";
import { useState } from "react";
import { useRef } from "react";

export default function Intelligent() {
    const [index, setIndex] = useState(0);  
    const [selectedAnswers, setSelectedAnswers] = useState(Array(questions2.length).fill(null)); //Array is an contructer with .fill()method  
    ...
    const nextQuestion = () => {
        ...
        if (selectedAnswers[index] === questions2[index].correctanswer) {
            setScore((prevScore) => prevScore + 1);  

        }

        setIndex((prevIndex) => (prevIndex + 1 <= questions2.length ? prevIndex + 1 : prevIndex));
        setAns("");  
        setCorrectAnswer(""); 
    };
    ...
    const Finish = () => {
        return (
            <>
                ...
                <h2 className="text-success">Your Score out of {questions2.length} Questions: {score}</h2>
                <h2 className="text-success">Each Question have 1 Mark</h2>  
                <button onClick={() => {
                    setIndex(0);
                    setScore(0);  
                    setSelectedAnswers(Array(questions2.length).fill(null));  
                    Scrollview();
                }} style={{color: "green"}} >
                    Try Again
                </button>
